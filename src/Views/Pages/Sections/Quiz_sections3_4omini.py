...
llm_key = "llm"
memory_key = "memory"
display_chat_history_key = "display_chat_history"
is_end_key = "is_end"
limit_try_count = 10
from typing import Callable, Protocol, Dict, Any

class ConditionFunction(Protocol):
  def __call__(self, created : dict, *args, **kwargs) -> bool:
    ...

def retry_decorator(max_try : int = 3, condition_function : ConditionFunction = None):
  def wrapper(func):
...
def generation_checker_condition(target_created_list : list):
  return target_created_list is not None and len(target_created_list) == 10

@retry_decorator(max_try=3, condition_function=generation_checker_condition)
def generate_question_by_target_word(target_word, target_word_category):
  ...
  return combined_questions.get("questions")

@retry_decorator(max_try=3, condition_function=generation_checker_condition)
def generate_hint_by_target_word(target_word, target_word_category):
	answer_hints = inference_generation_as_list(
		prompt_key="hints_generation_prompt",
		target_list_class=HintList,
		model_name="gpt-4o-mini",
		params={"quiz_answer": target_word, "count": 10}
	)
	return answer_hints.get("hints")
...
def safe_run_quiz():
	...
	display_chat_history = controller.get(display_chat_history_key)
	answer_word = controller.get(answer_word_key)
	answer_word_category = controller.get(answer_word_category_key)
	result = all([questions, hints, llm, memory, answer_word, answer_word_category, display_chat_history])
...
def safe_display_quiz():
	...
	answer_word = controller.get(answer_word_key)
	answer_word_category = controller.get(answer_word_category_key)
	return all([questions, hints, answer_word, answer_word_category])
...
def generation_button_callback(input_word, input_word_category):
	if input_word and input_word_category:
		...
		questions = generate_question_by_target_word(input_word, input_word_category)
...
def reset_button_callback():
	...
	controller.set(display_chat_history_key, None, overwrite=True)
...
def random_generation_callback():
	...
	questions = generate_question_by_target_word(target_word, target_word_category)
	hints = generate_hint_by_target_word(target_word, target_word_category)
	controller.set(questions_key, questions, overwrite=True)
...
def display_memory_on_chat(memory : MemoryController):
	if memory:
...
def init_quiz():
	if is_need_init_quiz() is False:
		...
		display_chat_history = MemoryController()
		controller.set(try_count_key, try_count, overwrite=True)
		controller.set(memory_key, memory, overwrite=True)
		controller.set(display_chat_history_key, display_chat_history, overwrite=True)
...
def is_need_init_quiz():
	...
	display_chat_history = controller.get(display_chat_history_key, None)
	return all([try_count, llm, memory, display_chat_history])
...
def quiz_logic():
	...
	if try_count == 0:
		...
		display_chat_history.add_message("assistant", message)
	if prompt := st.chat_input("질문을 입력하세요"):
		memory.add_message("user", prompt)
		display_chat_history.add_message("user", prompt)
	...
	if prompt:
		if is_right_answer(prompt, controller.get(answer_word_key)):
			message = response_right_word()
			memory.add_message("assistant", message)
			display_chat_history.add_message("assistant", message)
			controller.set(is_end_key, True, overwrite=True)
		else:
			picked_hint = controller.get(hints_key).pop() if controller.get(hints_key) else None
			response = llm.inference(prompt, memory)
			memory.add_message("assistant", response)
			response = f"{response} this is a hint: {picked_hint}"
			display_chat_history.add_message("assistant", response)
...
def chat_column():
	...
	with history_container:
		memory = controller.get(display_chat_history_key)
		display_memory_on_chat(memory)

def hint_button_callback():
	question_list = controller.get(questions_key)
